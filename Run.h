...
class Run{
    private:
        ...
        bool jocActiv;
    public:
        Run() : jucator(50, 90), wave(1), tura(0), jocActiv(true) {
        ...
        void spawnWave() {
            ...
            for (int i = 0; i < nrInamici; ++i) {
                ...
                float x = rand() % 100;
                float y = rand() % 100;
                inamici.push_back(std::make_unique<Inamic>(x, y, tip));
            }
        ...
        void actualizeaza() 
        {
            ...
            proiectileJucator.erase(std::remove_if(proiectileJucator.begin(), proiectileJucator.end(), 
                [](const Proiectil& p) { 
                    return p.getPozitie().getX() < 0 || p.getPozitie().getX() > 100 || 
                        p.getPozitie().getY() < 0 || p.getPozitie().getY() > 100; 
                }), proiectileJucator.end());
            
            proiectileInamici.erase(std::remove_if(proiectileInamici.begin(), proiectileInamici.end(), 
                [](const Proiectil& p) { 
                    return p.getPozitie().getX() < 0 || p.getPozitie().getX() > 100 || 
                        p.getPozitie().getY() < 0 || p.getPozitie().getY() > 100; 
                }), proiectileInamici.end());
            ...
            powerups.erase(std::remove_if(powerups.begin(), powerups.end(), 
                [this](Powerup& p) { 
                    return p.verificaColectare(jucator); 
                }), powerups.end());
